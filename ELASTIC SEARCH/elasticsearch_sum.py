...
### Real code
metrics = {"FreeStorageSpace":{"type":"float", "value":None}}
...
for k,vh in metrics.items():
    ...
    if (k == options.metric):
        ...
        try:
                res = conn.get_metric_statistics(60, start, end, k, "AWS/ES", "Sum", {"DomainName": options.domain_name,"ClientId": options.client_id})
               
        except Exception, e:
                print "status err Error running elb_stats: %s" % e.error_message
                sys.exit(1)
        if(len(res)>0):    
            sum = res[-1]["Sum"] # last item in result set
        else:
            sum = 0

        if vh["type"] == "float":
                metrics[k]["value"] = "%.4f" % sum
        if vh["type"] == "int":
                metrics[k]["value"] = "%i" % sum
